...
const MonthlyAnalysis = ({ month, year }: MonthlyAnalysisProps) => {
    const [monthlyAnalysis, setMonthlyAnalysis] =
        useState<MonthlyAnalysisType | null>(null);
    ...
    useEffect(() => {
        ...
        fetch(`/reports/monthly-analysis?month=${month}&year=${year}`)
            .then(async (response) => {
                const data =
                    (await response.json()) as ApiResponse<MonthlyAnalysisType>;
        ...
        return (
            <div className="flex items-center justify-center w-full h-full">
                <div role="status">
                    <svg
                        aria-hidden="true"
                        className="w-12 h-12 text-neutral-200 animate-spin dark:text-neutral-600 fill-primary"
                        viewBox="0 0 100 101"
                        fill="none"
    ...
    return (
        <>
                ...
                <BalanceCard
                    ...
                    icon={
                        monthlyAnalysis.top_category ? (
                            renderIcon(
                                monthlyAnalysis.top_category.icon,
                                16,
                                monthlyAnalysis.top_category.color,
                            )
                        ) : (
                            <ShoppingCart
                    ...
                    text={
                        monthlyAnalysis.top_category
                            ? monthlyAnalysis.top_category.name
                            : "Sem Categoria"
                    }
                    styleIcon={{
            ...
            <ReportsVariations
                data={monthlyAnalysis.expense_variation_by_category}
            />
        </>
    );
};
