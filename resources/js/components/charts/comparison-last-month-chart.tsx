...
const ComparisonLastMonthChart = ({
	...
	const chartConfig: ChartConfig = {
		previous_income: {
			label: first ? `Receitas de ${first.previous_month.month}` : 'Receitas mês anterior',
			color: "var(--chart-1)",
		},
		previous_expense: {
			label: first ? `Despesas de ${first.previous_month.month}` : 'Despesas mês anterior',
			color: "var(--chart-2)",
		},
		current_income: {
			label: first ? `Receitas de ${first.current_month.month}` : 'Receitas mês atual',
			color: "var(--chart-3)",
		},
		current_expense: {
			label: first ? `Despesas de ${first.current_month.month}` : 'Despesas mês atual',
			color: "var(--chart-4)",
		},
	};
