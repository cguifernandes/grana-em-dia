...
import { ApiResponse, CategoriesType, CategoryType, MonthlyAnalysisType } from "@/types/types";
...
import CategoryMonthlyComparisonChart, { MonthlyComparison } from "../charts/category-monthly-comparison-chart";
import { hexToRgba, renderIcon } from "@/utils/functions";
import ExpensesByCategoryChart, { CategoryExpense } from "../charts/expenses-by-category-chart";
import TrendAnalysis, { TrendData } from "../layout/trend-analysis";
...
const Categories = ({ month, year }: CategoriesProps) => {
    const [categoriesData, setCategoriesData]  = useState<CategoriesType | null>(null);
    ...
    useEffect(() => {
        ...
        fetch(`/reports/categories?month=${month}&year=${year}`)
            .then(async (response) => {
                const data = await (response.json()) as ApiResponse<CategoriesType>
